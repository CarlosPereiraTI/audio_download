...
url_field = Entry(frm, width=43, textvariable=url, borderwidth=2)
url_field.grid(column=1, row=0, padx=10, pady=10, columnspan=2)
...
ttk.Button(frm, text="Open Folder", command=open).grid(column=3, row=0)
...
ttk.Button(frm, text="Download Audio MP3", command=download_audio).grid(column=1, row=2)
...
ttk.Button(frm, text="Download Video MP4", command=video_download).grid(column=2, row=2)
...
# Help Menu
def about():
    tkinter.messagebox.showinfo(title="About", message="Tina | Youtube Downloader v1.0.1\n Your files are available in C:/Tina_Download")
...
# =====================================================================
# status bar
status = Label(root, text="v1.0.1", bd=1, relief=SUNKEN, anchor=E)
status.grid(row=4, column=0, columnspan=3, sticky=W+E)

# =====================================================================
# End program
ttk.Button(frm, text='Close', command=root.destroy).grid(column=3, row=4)
root.mainloop()